...
class Seq2seqDNCTrainer(nn.Module):
  ...
  def __init__(
    ...
    super(Seq2seqDNCTrainer, self).__init__()
    ...
    self.model = Seq2SeqDNC(self.src_lang, self.targ_lang, self.n_layers, self.hidden_size,
                            self.teacher_forcing_ratio, self.attention_type, self.gpu_id,
                            bidirectional_encoder, bidirectional_decoder,
                            mem_size, cell_size, read_heads,
                            dropout)
