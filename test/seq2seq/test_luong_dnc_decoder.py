...
def test_luong_dnc_decoder():
  ...
  dec = LuongAttnDecoderDNC('general', 1024, 2, vocab_size=target_vocab_size)
  hidden = h
  # sentences = []
  input = cudavec(np.array([SOS] * 30, dtype=np.long)).unsqueeze(1)
  for x in range(input.size()[1]):
    o, hidden, att = dec(input, y, None)
    assert o.size() == T.Size([30, 1, target_vocab_size]), \
        'Output has size batch_size * 1 * target_vocab_size'
    # assert hidden[0].size() == T.Size([2, 30, 1024]), \
    #     'Hidden has size nr_layers * batch_size * hidden_size'
    # assert att.size() == T.Size([30, 1, 14]), \
